...
class PromptLearner(nn.Module):
    def __init__(self,
        ...
        #  defines the dimension of the embeddings, based on the final layer of the clip_model
        ...
        for cls in prompts:
            for position in class_token_position:
                if CSC:
                    ctx_vectors = torch.empty(len(prompts[cls]), n_ctx, ctx_dim).to(device) # [5,8,768]
                else:
        ...
        # enable optimizing dimension similar to ctx_vectors
        ...
        for cls in embeddings:
            self.register_embeddings['{}_token_prefix'.format(cls)]=embeddings[cls][:, :1, :] #[5,1,768]
            self.register_embeddings['{}_token_suffix'.format(cls)]=embeddings[cls][:, 1 + n_ctx :, :] # [5,68,768]
...
class PromptMaker(nn.Module):
    ...
    def __init__(self,
        ...
        # clip_model = clip_model.text
        super().__init__()
        assert 'normal' in prompts or 'abnormal' in prompts

        for position in class_token_position:
            assert  position in ['end','middle','front']

        self.prompt_learner = PromptLearner(prompts=prompts, n_ctx= n_ctx, CSC=CSC, class_token_position=class_token_position, clip_model=clip_model)
